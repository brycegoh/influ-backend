...
const verifyToken = (req, res, next) => {
    ...
    const userId = checkTokenForChanges( accessToken, PUB_KEY )
    console.log(userId)
    console.log(users)
    users._findOne({query:{"_id":userId}})
    .then(userData=>{
        console.log("userData")
        const refreshTokenValid = checkRefreshToken( refreshToken, `${PRIV_KEY_Rf}${userData.password}`, userData._id )
        if(refreshTokenValid){
            ...
            req.cookie('refresh_token', newRefreshToken, {httpOnly:true, sameSite:true});
            return next();
        }
        if(!refreshTokenValid){
...
module.exports = {
    ...
    signRfToken,
    verifyToken,
    cookieExtractor,checkTokenForChanges,checkRefreshToken,PUB_KEY
}